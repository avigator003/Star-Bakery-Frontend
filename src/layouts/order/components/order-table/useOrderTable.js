...
const useOrderTable = (
  ...
  const handleDownloadInvoice = useCallback(
    ...
    [handleShowSpinner, handleCloseSpinner, finalOrderStatus]
  );
      ...
      rowData?.map((row) => ({
        ...
        paymentStatus: (
          <>
            ...
            />
            <Select
              disabled={orderStatus === "Completed" || row?.status === "Completed"}
              defaultValue={row?.paymentStatus}
              onChange={(event) => handleChangePaymentStatus(row._id, event.target.value)}
            ...
            >
              ...
              <MenuItem value="Unpaid">
                <div style={{ color: "#F2A000", fontWeight: "bolder" }}>Unpaid</div>
              </MenuItem>
              {row?.paymentStatus !== "Paid" && (
                <MenuItem value="Partial Payment">
                  <div style={{ color: "green", fontWeight: "bolder" }}>Partial Payment</div>
                </MenuItem>
              )}
            </Select> */}
          </>
        ...
        //     handleDownloadAction={() => handleDownloadInvoice(row?.status, row._id)}
